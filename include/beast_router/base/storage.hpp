...
class storage {
 ...
 public:
  ...
  template <class... OnRequest,
            std::enable_if_t<
                utility::is_all_true_v<
                    (std::is_invocable_v<OnRequest, const message_type &,
                                         context_type &, const std::smatch &> ||
                     std::is_invocable_v<OnRequest, const message_type &,
                                         context_type &> ||
                     std::is_invocable_v<OnRequest, context_type &>)...> &&
                    sizeof...(OnRequest) >= 1,
                bool> = true>
  storage(OnRequest &&...on_request) : m_clbs{} {
    ...
    for (size_t idx = 0; idx < size; ++idx) {
      utility::tuple_func_idx(
          idx, tuple, std::make_index_sequence<size>{},
          [&](auto &&entry) -> bool {
            m_clbs.push_back(
  ...
  size_t size() const { return m_clbs.size(); }

  bool begin_execute(const message_type &request, context_type &&ctx,
                     std::smatch &&match) {
 ...
 private:
  struct callback {
    ...
    virtual bool operator()(const message_type &, context_type &,
                            const std::smatch &) = 0;
  };
  ...
  struct callback_impl : callback {
    ...
    template <class F = Func,
              std::enable_if_t<
                  std::is_invocable_v<F, const message_type &, context_type &,
                                      const std::smatch &>,
                  bool> = true>
    callback_impl(Func &&func) {
      m_func = std::bind<return_type>(
          std::forward<Func>(func), std::placeholders::_1,
    ...
    callback_impl(Func &&func) {
      m_func =
          std::bind<return_type>(std::forward<Func>(func),
                                 std::placeholders::_1, std::placeholders::_2);
    }

    template <
        class F = Func,
        std::enable_if_t<std::is_invocable_v<F, context_type &>, bool> = true>
    callback_impl(Func &&func) {
      m_func = std::bind<return_type>(std::forward<Func>(func),
                                      std::placeholders::_2);
    }

    bool operator()(const message_type &request, context_type &ctx,
                    const std::smatch &match) override {
    ...
    std::function<return_type(const message_type &, context_type &,
                              const std::smatch &)>
        m_func;
  };
