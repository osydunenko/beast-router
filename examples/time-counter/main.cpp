...
int main(int, char **) {
  ...
  ::router.get(
      R"(^.*\.(js|html|css)$)", [](const server_session::message_type &rq,
                                   server_session::context_type &ctx) {
        // Request path must be absolute and not contain ".."
        if (rq.target().empty() || rq.target()[0] != '/' ||
  ...
  ::router.get(R"(^/$)", [](const server_session::message_type &rq,
                            server_session::context_type &ctx) {
    ctx.send(make_moved_response(rq.version(), "/index.html"));
  });

  /// Handle the "update" request
  ::router.get(R"(^/update$)", [](const server_session::message_type &rq,
                                  server_session::context_type &ctx) {
    ...
    ctx.send(make_string_response(http::status::ok, rq.version(), i_str.str()));
  });
