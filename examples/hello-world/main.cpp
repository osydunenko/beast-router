...
{
    /// routing table
    beast_router::server_session::router_type router {};

    /// Add the callback and link to index ("/") requests patterns
    router.get(R"(^/$)", [](const beast_router::server_session::message_type& rq, beast_router::server_session::context_type& ctx) {
    ...
    beast_router::http_listener::on_error_type on_error = [event_loop](boost::system::error_code ec,
                                                              std::string_view) {
    ...
    beast_router::http_listener::on_accept_type on_accept = [&on_error, &router](beast_router::http_listener::socket_type socket) {
        beast_router::server_session::recv(std::move(socket), router, 1s, on_error);
    };

    /// Start listening
    beast_router::http_listener::launch(*event_loop, { address, port }, std::move(on_accept), std::move(on_error));
